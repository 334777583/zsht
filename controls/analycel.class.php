...
class analycel{
	...
	private function ReadExcel($path){
		...
		for($j = 1;$j <= $highestRow; $j++){
			...
			// $result[$j]['f_id'] = $PHPExcel->getActiveSheet()->getCell("H".$j)->getValue();//获取道具名
			// $result[$j]['f_name'] = $PHPExcel->getActiveSheet()->getCell("I".$j)->getValue();//获取道具id
		}
		return $result;
		exit;
	}
	...
	private function update($data, $id){
		...
		$f = $db->rquery($sql);
		if(!$f) return false;
		$sql = 'insert into goods_detail(g_code,g_name) values ' . $data;
		$f = $db->rquery($sql);
		if(!$f) 
			return false;
		else	
