		...
		if($ip) {
			...
			if(true) {	
				...
				if(!$point){
					...
					$SingResult = $point ->fquery("SELECT p.AccountId,p.RoleName,p.level,p.CreateTime,p.LoginTime FROM player_table p LEFT JOIN game_user g ON g.id=p.AccountId WHERE p.level > 0 ORDER BY p.{$_POST['sort']} DESC");	
				}
				if(!empty($SingResult)){//组装数据
					...
					foreach($SingResult as $key => $value){
						...
						if (strtotime(date('Y-m-d'))-$SingResult[$key]['LoginTime'] > 3600*24*3) {
							$SingResult[$key]['statis'] = 2;
						}else{
							$SingResult[$key]['statis'] = 1;
						}
						//当时间不存在的时候 默认为今天
						$create_time = date('Y-m-d H:i:s',$SingResult[$key]['CreateTime']);
						$last_down_time = date('Y-m-d H:i:s',$SingResult[$key]['LoginTime']);
						

						//组装数据
						$SingResult[$key]['CreateTime'] = $create_time;//注册时间
						$SingResult[$key]['LoginTime'] = $last_down_time;//最后下线时间
