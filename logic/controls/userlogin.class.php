	...
	public function getdata(){
		...
		if($list == '') {			//没有记录时，默认为0
			...
			foreach($dateArr as $dates){
				$login_all = $obj -> table("online_sec") -> field('o_userid,left(o_date,10) as o_date')-> where("left(o_date,10) <= '".$dates."'") -> group('o_userid,left(o_date,10)') -> select();	//获取所有登录信息，计算2登等信息（不是同一天有2次登录的算2登）
			...
			*/
			...
			if(is_array($tempArr)) {
				foreach($tempArr as $time => $num) {
					$d_arr[date('Y-m-d H:i:s', $time)] = $num;
				}
				$tempArr = $d_arr;
				unset($d_arr);
			}
