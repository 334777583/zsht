	...
	public function getLevel() {
		...
		if($ip) {
			...
			if(true) {	
				...
				// list($sip) = autoConfig::getConfig($ip);
				$obj = F($t_conf['zs']['db'], $t_conf['zs']['ip'], $t_conf['zs']['user'], $t_conf['zs']['password'], $t_conf['zs']['port']);//链接对应数据表
				if(!$obj){
					...
					$CountSql = 'select '.$Sql_Str.' from player_table where LogoutTime <= "'.$endday.'"';
