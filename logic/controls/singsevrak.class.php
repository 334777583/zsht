...
class singsevrak{
	...
	public function SingSevRak() {
		...
		if($ip) {
			...
			if(true) {	
				...
				$SingResult = $Obj -> table('sing_rank') -> limit($start_limit,$end_limit) -> order($sort_str) -> select();
				$total = $Obj -> table('sing_rank') -> total();
			...
			if(true) {	
				...
				if(!empty($SingResult)){//组装数据
					...
					foreach($SingResult as $key => $value){
						...
						//当时间不存在的时候 默认为今天
						$create_time = $this -> DateFormat($data[$key]['create_time']);
						$last_down_time = $this -> DateFormat($data[$key]['last_down_time']);
						
						//组装数据
						$data[$key]['create_time'] = $create_time;//注册时间
						$data[$key]['last_down_time'] = $last_down_time;//最后下线时间
	...
	public function writeExcel(){
		$ip = get_var_value('ip');
		$Obj = D(GNAME.$ip);
		$SingResult = $Obj -> table('sing_rank') -> order('s_money desc , s_level desc')->limit(0,5000) -> select();
