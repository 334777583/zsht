...
AmCharts.AmChart = AmCharts.Class({
	...
	drb: function() {
		...
		// if (e != b) {
			...
			// f = AmCharts.text(this.container, 'www.yanjnilong.com', "#000000", "Verdana", 11, "start");
			// f.translate(7, 9);
			// e = this.container.set([e]);
...
AmCharts.AmGraph = AmCharts.Class({
	...
	createBullet: function(a, b, d) {
		...
		l && (j && j.remove(), "function" == typeof l ? (j = new l, j.chart = this.chart, a.bulletConfig && (j.availableSpace = d, j.graph = this, a.bulletConfig.minCoord = this.minCoord - d, j.bulletConfig = a.bulletConfig), j.write(e), j = j.set) : (this.chart.path && (l = this.chart.path +
				...
				if (d.mouseIsOver || z || C || this.forceShow) if (this.forceShow = !1, C) {
					...
					if (I && this.valueBalloonsEnabled && v && d.balloon.enabled) {
						...
						if (this.oneBalloonOnly) {
							...
							u.showBalloon && (!u.hidden && u.balloonText) && (t = s.axes[u.valueAxis.id].graphs[u.id], w = t.y, isNaN(w) || (r ? Math.abs(e - w) < j && (j = Math.abs(e - w), O = u) : Math.abs(f - w) < j && (j = Math.abs(f -
							w), O = u)));
	...
	drawChart: function() {
		...
		if (AmCharts.ifArray(b)) {
			if (0 < a.realWidth && 0 < a.realHeight) {
				...
				s,
				p,
				r = AmCharts.toNumber(a.labelRadius),
				q = a.measureMaxLabel();
