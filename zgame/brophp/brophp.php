 ...
 * ******************************************************************************/
	...
	//设置session生存时间
	defined('SESSION_LIFETIME') && ini_set('session.cookie_lifetime', SESSION_LIFETIME * 3600 * 24);
	defined('SESSIOIN_GC_MAXLIFETIME') && ini_set('session.gc_maxlifetime', SESSIOIN_GC_MAXLIFETIME);
	...
	if(!defined("SESSIONTYPE"))  define("SESSIONTYPE",   false);
	//如果Memcach开启且使用Memcache做session存储，设置将Session信息保存在Memcache服务器中
	if(defined("USEMEM") && SESSIONTYPE == 'memcache'){
		MemSession::start($mem->getMem());
		Debug::addmsg("开启会话Session (使用Memcache保存会话信息)"); //debug
	}elseif(SESSIONTYPE == 'mysql'){
		MysqlSession::start();
		Debug::addmsg("<font color='red'>开启会话Session</font> (使用Mysql保存会话信息)");
	}else{
		session_start();
