 ...
 * ******************************************************************************/
	class MemcacheModel {
		private $mc = null;
		public  $limit_cache_time = 600; //缓存10分钟
		/**
		...
		private function addKey($tabName, $key){
			...
			if(!in_array($key, $keys)) {
				...
				$this->mc->set($tabName, $keys, MEMCACHE_COMPRESSED, $this->limit_cache_time);
		...
		function addCache($tabName, $sql, $data){
			...
			if($this->addKey($tabName, $key)){
				$this->mc->set($key, $data, MEMCACHE_COMPRESSED, $this->limit_cache_time);
			}
		}
		/**
		 * 获取memcahce中保存的数据
