 ...
 * ******************************************************************************/
	class Dmysqli extends DB{
		 ...
		 * Description: 增加构造函数，解决中文乱码
		 * Permissions: Public
		 * Author: 陈 飞
		 * Date: 2012年05月05日 15:17
		 **/
		...
		function query($sql, $method,$data=array()){
			...
			self::writeFile('slq_log.log', '['.date('Y-m-d H:i:s').'] '.$memkey ."\r\n", 'a+');
			return $returnv;
		}
		
		private function writeFile($file,$str,$mode='w')
		{
			$oldmask = @umask(0);
			$fp = @fopen($file,$mode);
			@flock($fp, 3);
		 ...
		 * @param	NULL
		 */
		public function query_nothing($sql){
			if(!isset($this -> r_mysqli)){
				$this -> r_mysqli=self::r_connect();
			}
			if($this -> r_mysqli){
				return $result=$this -> r_mysqli -> query($sql);
			}else{
				return false;
			}
		}
